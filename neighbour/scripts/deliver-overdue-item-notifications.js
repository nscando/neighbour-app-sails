module.exports = {
  ...
  description:
    "Send a message to the owner of a thing requesting to borrow it, and mark it as borrowed.",
  ...
  inputs: {
    ...
    expectedReturnAt: {
      type: "number",
      description:
        "A JS timestamp (epoch ms) representing the requested return time.",
    ...
    pickupInfo: {
      type: "string",
      description:
        "The pickup time information to use in the message sent to the owner.",
  ...
  exits: {
    success: {
      description:
        "A request has been sent to the item's owner, and the item is now marked as borrowed.",
    },
  },
  ...
  fn: async function ({ id, expectedReturnAt, pickupInfo }) {
    ...
    var itemBelongsToFriend = _.any(this.req.me.friends, {
      id: borrowing.owner.id,
    });
    ...
    var formattedExpectedReturnAt = moment(expectedReturnAt).format(
      "dddd, MMMM Do"
    );
    var formattedPickupInfoText =
      pickupInfo.charAt(0).toLowerCase() + pickupInfo.slice(1);
