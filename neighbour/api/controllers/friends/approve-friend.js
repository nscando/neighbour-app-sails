module.exports = {
  ...
  description:
    "Approve the pending friend request from the user with the specified ID.",
  ...
  exits: {
    notFound: {
      description:
        "There is no pending friend request from a user with that ID.",
  ...
  fn: async function ({ id }) {
    ...
    await User.addToCollection(id, "friends").members([this.req.me.id]);
    await User.addToCollection(this.req.me.id, "friends").members([id]);
    ...
    await User.removeFromCollection(id, "outboundFriendRequests").members([
      this.req.me.id,
    ]);
  },
};
